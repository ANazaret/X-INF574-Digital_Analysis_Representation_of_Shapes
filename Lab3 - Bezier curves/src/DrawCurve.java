...
public class DrawCurve extends PApplet implements ActionListener {
    ...
    public void initButton() {
        ...
        button2 = new Button("Switch Bezier Rendering (subdive / bernstein)");
        add(button2);
        ...
        button4 = new Button("Transform (None, Affine, Projective)");
        add(button4);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == button2) {
            renderSubdiv = !renderSubdiv;
            System.out.println("Button 2 was pressed");
        }

        if (e.getSource() == button4) {
            renderTransf = (renderTransf + 1) % 3;
            for (Curve c : schemes) {
                if (c instanceof Bezier) {
                    switch (renderTransf) {
                        case 0:
                            ((Bezier) c).transformation = null;
                            break;
                        case 1:
                            ((Bezier) c).transformation =
                                    new Transformation_2(new Vector_2(400, 0)).compose(new Transformation_2(0.3));
                            break;
                        case 2:
                            double[][] tmp = {{1,0,300},{0,1.5,100},{0,0.001,0.8}};
                            Matrix m = new Matrix(tmp, 3, 3);
                            ((Bezier) c).transformation =
                                    new Transformation_2(m);
                            break;
                    }
                }
            }
